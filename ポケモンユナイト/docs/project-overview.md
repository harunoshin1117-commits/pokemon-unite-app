...
`abilityDisplayData` は、攻撃側UIに特性名・短い説明・画像パス・補足を表示するための表示専用データです。現時点ではエースバーンの `もうか` を表示対象にしており、特性の発動判定、ステータス変更、火種スタック、ダメージ計算には接続していません。表示データがないポケモンでは、UI上に「特性情報は準備中です」と表示します。

注意: `pokemonsList` は複数ポケモンを含む巨大配列です。行番号や前後の `stats` だけで読むと、別ポケモンのステータスを誤読しやすいです。ステータス確認時は必ず `id` で対象ポケモンを特定してから読んでください。主な `id` は、ピカチュウが `Pikachu`、ゲッコウガが `Greninja`、エースバーンが `Cinderace` です。

ステータス確認だけを行う場合は、手で行番号を追わずに専用コマンドを使います。

```bash
npm run stats -- Cinderace 15
npm run stats -- Pikachu 15
npm run stats -- Greninja 15
```

第1引数は `pokemonData.js` の `id`、第2引数はレベルです。レベルを省略すると、そのポケモンの全レベルの `stats` をJSONで表示します。
...
エースバーンの特性 `もうか` は、`passiveEffects` にデータのみ追加しています。HP50%以下時の急所率+10%・通常攻撃速度+20%、通常攻撃1個/強化通常攻撃2個/技2個の火種付与、5個時の発火追加ダメージ式を持ちます。ゲーム内検証により、火種持続5秒、5個以上で発火、発火時は5個だけ消費して余剰スタックが残ること、本体ダメージ表示後に火種追加ダメージが表示されることを measured として記録しています。現在の単発計算画面では、自分の現在HP、時間経過、クールダウン、対象ごとの火種スタックを扱っていないため、計算処理にはまだ接続していません。内部処理順、再付与時のタイマー更新方式、複数対象ごとの独立保持、発火追加ダメージの丸め順は未確認です。

エースバーンのユナイト技 `Blazing Bicycle Kick` の減少HP参照ダメージは、ゲーム内検証により、発動入力時ではなく命中時点の相手HPを参照する挙動を measured として調査データへ記録しています。命中時点とは本体ダメージ反映前のHPであり、発動から命中までの自然回復によるHP変化は追加ダメージ量へ反映されます。命中前に別攻撃でダメージを受けた場合、回復技でHPが変化した場合、小数点および防御補正の正確な丸め段階は needsRecheck のままです。現行の計算処理にはまだ接続していません。
...
- `updateAttackerSummaryCard()` による攻撃側ポケモン要約カードの更新
- `updateAbilityDisplay()` による攻撃側特性表示欄の更新
